...
##Bài 8 Thêm template admin. đăng nhập hệ thống
...
8. Coppy ma4 token trong appsetting cua api vao admin
9. Chạy multi api . admin
- đăng nhập lấy về dc token -->>>ok
##chứng thực với cookies không dùng identity
...
3. Viết phương thức giải mã token bên controller user admin
...
4.chỉnh sửa lại phương thức post vào login()
    [HttpPost]
        public async Task<IActionResult> Login(LoginRequest request)
        {
            if (!ModelState.IsValid)
...
5. chỉnh sửa lại phương thức login get
        [HttpGet]
...
6. [Authorize] trang HomeController admin không cho phép vào khi chưa đăng nhập
7. Cấu hình trong starUp
...
8. Bên seoShopSolution.application userservice lấy ra  new Claim(ClaimTypes.Name, user.UserName),
